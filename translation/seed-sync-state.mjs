...
function localePages(loc) {
  const out = execFileSync(
    "git",
    ["ls-files", `translations/${loc}/site/`],
...
function curatedIntersection() {
  ...
  if (partial.length) {
    console.warn(
      `WARNING: ${partial.length} page(s) present in some but not all locales ` +
        `(excluded from curated list):\n  ${partial.join("\n  ")}`,
    );
  }
  return inAll.sort();
}
...
if (existsSync(manifestPath) && !process.argv.includes("--force")) {
  try {
    ...
    const risky = Object.values(prev).some((byPage) =>
      Object.values(byPage).some((e) => e && (e.edited === true || (e.mode && e.mode !== "seed"))),
    );
    if (risky) {
      console.error(
  ...
  } catch { /* unreadable/empty manifest — safe to (re)seed */ }
}
