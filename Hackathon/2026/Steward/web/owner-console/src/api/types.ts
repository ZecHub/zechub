...
/** POST /vault/:id/sync — a real network scan of the vault's UFVK (~15s). */
...
export interface DemoVaultRequest {
  ...
  network?: Network
  /** The owner's Ed25519 heartbeat public key (32-byte hex). The console generates the keypair
   *  client-side and sends only the PUBLIC key — the coordinator never sees the secret. */
