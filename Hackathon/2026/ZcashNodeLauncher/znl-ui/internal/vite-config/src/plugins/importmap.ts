/**
 * Reference https://github.com/jspm/vite-plugin-jspm, adjusted to required functionality
 */
...
async function getShimsUrl(provide: string) {
  ...
  const providerShimsMap: Record<string, string> = {
    ...
    // The next two CDNs are unstable. Not for the time being.
...
async function viteImportMapPlugin(
  ...
  return [
    ...
    {
      buildEnd() {
        // If not generated, eject error to prevent turbo cache
        if (!installed && !isSSR) {
