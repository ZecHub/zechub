...
const [Modal, modalApi] = useVbenModal({
  ...
  onOpenChange(isOpen) {
    if (isOpen) {
      // When open, load the load. Later, the CropperImage component is loaded and automatically closes the load (by handleReady)
      modalLoading(true);
      const img = new Image();
      img.src = src.value;
      ...
      // Clear the right preview when closing
      previewSource.value = '';
      modalLoading(false);
    }
  },
...
function handleReady(cropperInstance: CropperType) {
  ...
  // The canvas is loaded and closed.
  modalLoading(false);
}
...
async function handleOk() {
  ...
  if (uploadApi && isFunction(uploadApi)) {
    if (!previewSource.value) {
      message.warn('No pictures selected');
      return;
    }
