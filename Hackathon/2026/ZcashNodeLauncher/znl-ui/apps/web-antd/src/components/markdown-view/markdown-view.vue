...
const { copy } = useClipboard(); // Initialize coppy to pasteboard
...
const md = new MarkdownIt({
  highlight(str, lang) {
    if (lang && hljs.getLanguage(lang)) {
      try {
        const copyHtml = `<div id= "copy" data-copy="${str}' style="position: absolute; right: 10px; top: 5px; color: #fff; cursor: point;" copy</div>`;
...
.markdown-view {
  ...
  ol > li {
    ...
    // Expressions, repair an ordered list serial number display of incomplete questions
    // &:nth-child(n + 10) {
    //     margin-left: 30px;
    // }
