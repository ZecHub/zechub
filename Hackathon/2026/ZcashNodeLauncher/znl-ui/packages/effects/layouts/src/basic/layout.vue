...
/**
 * Packing menu, translation menu name * @param menus original menu data * @param deep whether to package deep. For double-column layouts, only the first layer of packaging is required because deeper data are repackaged in the extension menu
 */
...
function autoCollapseMenuByRouteMeta(route: RouteLocationNormalizedLoaded) {
  // Only effective in double-column mode
  if (
...
// Update language, refresh page
// i18n.global.locale will not be updated until after the change of preference.app.locale, so watchpreference.app.locale is not appropriate and the language configuration may not be fully loaded at the time the page is refreshed
