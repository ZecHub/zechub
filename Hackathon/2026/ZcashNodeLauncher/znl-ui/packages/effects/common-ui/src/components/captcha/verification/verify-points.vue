...
const checkNum = ref(3); // The number of words to click by default
const fontPos = reactive<any[]>([]); 
const checkPosArr = reactive<any[]>([]); // Coordinates on which the user clicks
const num = ref(1); // Number of clicks
const pointBackImgBase = ref(); // Background picture obtained from backend
const poinTextList = ref<any[]>([]); // Font order to click backend returned
const backToken = ref(); // Token value returned from backend
...
function canvasClick(e: any) {
  ...
  if (num.value === checkNum.value) {
    ...
    // Wait until the creation coordinates are finished.
    setTimeout(() => {
