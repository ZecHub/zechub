...
class SSE {
  ...
  /**
   * SSE Request Method * @param url - Request URL * @param Data - Request Data * @param requestOptions - SSE Request Options
   */
...
function safeJoinUrl(baseUrl: string | undefined, url: string): string {
  if (!baseUrl) {
    return url; // No baseUr, return directly to url
  }

  // If url is an absolute address, return directly
  if (/^https?:\/\//i.test(url)) {
    return url;
  }

  // If baseUrl is full URL, use new URL
  if (/^https?:\/\//i.test(baseUrl)) {
    return new URL(url, baseUrl).toString();
  }

  // Otherwise, spell it as a path.
