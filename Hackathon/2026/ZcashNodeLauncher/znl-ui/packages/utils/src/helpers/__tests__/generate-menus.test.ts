...
describe('generateMenus', () => {
  ...
  const mockRoutes = [
    {
      meta: { icon: 'home-icon', title: 'Home Page' },
      name: 'home',
      path: '/home',
    },
    {
      meta: { hideChildrenInMenu: true, icon: 'about-icon', title: 'About' },
      name: 'about',
      path: '/about',
      children: [
        {
          path: 'team',
          name: 'team',
          meta: { icon: 'team-icon', title: 'Team' },
        },
      ],
    },
  ] as RouteRecordRaw[];

  // Simulate the example of the Vue router
  const mockRouter = {
    getRoutes: vi.fn(() => [
      { name: 'home', path: '/home' },
