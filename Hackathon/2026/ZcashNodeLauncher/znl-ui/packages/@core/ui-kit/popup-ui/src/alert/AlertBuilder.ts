  ...
  return new Promise((resolve, reject) => {
    ...
    if (arg1) {
      if (isString(arg1)) {
        ...
        // Merge into options if the second parameter is an object
    ...
    // Creates a reference for accessing examples in return
    const alertRef = { container, instance: null as any };

    const props: AlertProps & Recordable<any> = {
      onClosed: (isConfirm: boolean) => {
        // Remove component examples and create all doms (restore page to pre-open status)
        // Remove this example from the aerts array
        ...
        // Parsing Promise, passing user-operable results
        if (isConfirm) {
          resolve();
        } else {
...
export async function vbenPrompt<T = any>(
  ...
  // Content functions that are recalculated every time you rewrite
  const contentRenderer = () => {
    ...
    // Returns the array that contains static content and input components
    return h(
      'div',
  ...
  const props: AlertProps & Recordable<any> = {
    ...
    // Use function format, recalculating each rendering
