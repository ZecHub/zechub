...
// Whether the side is in the Hover Expanding Menu
...
/**
 * Dynamic Retrieval Side Area Visible
 */
...
/**
 * Whether sidebar mode includes mixed side
 */
...
// Calculating tabbar styles
const tabbarStyle = computed((): CSSProperties => {
  let width = '';
  let marginLeft = 0;

  // If not mixed navigation, the width of Tabbar is 100%
  if (!isMixedNav.value || props.sidebarHidden) {
    width = '100%';
  } else if (sidebarEnable.value) {
    // The width of the sidebar when the mouse is on the sidebar and the sidebar is expanded
    const onHoveringWidth = sidebarExpandOnHover.value
      ? props.sidebarWidth
      : getSideCollapseWidth.value;

    // Set marginLeft, depending on whether the sidebar folds
    marginLeft = sidebarCollapse.value
      ? getSideCollapseWidth.value
      : onHoveringWidth;

    // Sets the width of the tabbar, calculated as 100% less the width of the sidebar
