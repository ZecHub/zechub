...
export class FormApi {
  ...
  // Last click on the form value at the time of submission
  ...
  /**
   * Get Field Component Example * @param fieldName Field Name * @returns Component Example
   */
  ...
  /**
   * Gets the current focus field, returns unfocused if no focus field
   */
  getFocusedField() {
  ...
  /**
   * Remove Form Item by Field Name * @paramfields
   */
  ...
  /**
   * Scroll to the first error field * @param error
   */
  scrollToFirstError(errors: Record<string, any> | string) {
    ...
    // If the name attribute is not found, try to find it by component quotes, you will not normally come here, fearing that one day the Vee-validate will change the name attribute.
    if (!el) {
    ...
    if (el) {
      // Scroll to the wrong field, add some deviations to ensure that the field is fully visible
      el.scrollIntoView({
        behavior: 'smooth',
        block: 'center',
        inline: 'nearest',
  ...
  /**
   * Set Form Disable Status: Form needs to control disablement status * @author potato source * @paramdisabled for non-Modal use
   */
  ...
  /**
   * Sets the load status of form submission buttons: Form is required to control the load status * @author potato source * for non-Modal use of Form whether to load @paramloading
   */
  ...
  /**
   * Sets the form value * @paramfields record * @param filterfields filters fields not defined in schema by default as True * @param should Validate
   */
  async setValues(
    ...
    /**
     * The consolidation algorithm needs to be improved and the current algorithm does not support the value of the object type. * The value type of the date-time-related component is the dayjs object * date-time-related component may be the value type of the date object * The two types above need to be excluded from deep consolidation
     */
  ...
  private handleMultiFields = (originValues: Record<string, any>) => {
    ...
    const processFields = (fields: string[], separator: string = ',') => {
      this.processFields(fields, separator, originValues, (value, sep) => {
        if (Array.isArray(value)) {
          ...
          // Deal with empty string situations
          if (value === '') {
            return [];
          }
          // Dealing with complex separator situations
    ...
    // Process simple array formats [`field1', 'field2', ';'] or [`field1', 'field2']
    ...
    // Handle embedded array format['field1', ';']
    arrayToStringFields.forEach((fieldConfig) => {
      if (Array.isArray(fieldConfig)) {
        const [fields, separator = ','] = fieldConfig;
        // By type, Fields should always be a string array
