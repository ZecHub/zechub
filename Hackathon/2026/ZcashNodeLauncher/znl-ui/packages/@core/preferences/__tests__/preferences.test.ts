...
describe('preferences', () => {
  ...
  // Simulate Windows.matchMedia Method
  vi.stubGlobal(
    'matchMedia',
  ...
  it('initializes preferences with overrides', async () => {
    ...
    // Waiting for the tremors to be completed.
  ...
  it('resets preferences to default', () => {
    // Let's update some preferences first.
    preferenceManager.updatePreferences({
      theme: {
        mode: 'light',
      },
    ...
    // And then reset the preferences.
  ...
  it('ignores updates with invalid preference value types', () => {
    ...
    preferenceManager.updatePreferences({
      app: { isMobile: 'true' as unknown as boolean }, // Error Type
    });
