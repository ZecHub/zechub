...
import { usePrices, usePoolsHistory, useStatus, useCurrentPrice } from '@/hooks/useZecData';
...
export function SupplyChart() {
  ...
  const { data: priceData, loading: priceLoading, error: priceError, refetch: refetchPrices } = usePrices(timeRange);
  const { data: poolsData, loading: poolsLoading, error: poolsError, refetch: refetchPools } = usePoolsHistory(timeRange);
  ...
  const loading = priceLoading || poolsLoading;
  const error = priceError || poolsError;
  ...
  const nowData = useCallback(() => {
    ...
    const totalZec = (pools?.transparent ?? 0) + (pools?.sprout ?? 0) + (pools?.sapling ?? 0) + (pools?.orchard ?? 0);
    const value = currency === 'zec' ? totalZec : totalZec * (currentPrice ?? 0);
    return {
      time: now.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: '2-digit' }),
      value: formatValue(value),
    };
  }, [statusData, currentPrice, currency, formatValue]);

  const refetch = useCallback(() => {
    refetchPrices();
    refetchPools();
  }, [refetchPrices, refetchPools]);
  ...
  useEffect(() => {
    if (error || (!loading && !poolsData)) {
      setIsCharting(false);
    }
  }, [error, loading, poolsData]);

  useEffect(() => {
    // Need pools data; price data only needed for USD mode
    if (!poolsData || !containerRef.current) return;
    ...
    const chartTimeout = setTimeout(() => {
      ...
      poolsData.forEach(p => {
        ...
        const totalZec = p.transparent + p.sprout + p.sapling + p.orchard;

        if (currency === 'zec') {
          timestamps.push(ts / 1000);
          supplyValues.push(totalZec);
        } else {
          const price = getPriceForTimestamp(ts);
          if (price !== null) {
            timestamps.push(ts / 1000);
            supplyValues.push(totalZec * price);
          }
        }
      });
      ...
      if (style === 'pixel') {
        ...
        for (let i = 0; i < timestamps.length; i += step) {
          ...
          plotSupply.push(supplyValues[i]);
        }
      }
      ...
      chartRef.current = new uPlot(opts, [plotTimestamps, plotSupply], container);
      setIsCharting(false);
  ...
  }, [priceData, poolsData, style, showAxes, currency, timeRange, getPriceForTimestamp]);
  ...
  // Show awaiting message if pools data isn't available
  const needsPoolsApi = poolsError || (!poolsLoading && !poolsData);
    ...
    <div className="flex flex-col h-full">
      ...
      {showLive && displayData && (
        <div className="flex justify-end gap-4 mt-2" style={{ fontSize: 'var(--font-size-label)' }}>
          ...
          <span className={currency === 'usd' ? 'text-[var(--accent-gold)]' : 'text-[var(--accent-green)]'}>{displayData.value}</span>
        </div>
      )}

      <style jsx global>{`
        .uplot .u-cursor-x, .uplot .u-cursor-y { border-color: rgba(244, 183, 40, 0.6) !important; }
