...
class AddressCarouselState extends State<AddressCarousel> {
  ...
  void updateAddresses() {
    ...
    // Determine the initial index based on requested initial address mode
    if (widget.initialAddressMode != null) {
      final idx = addressModes.indexOf(widget.initialAddressMode!);
...
class _QRAddressState extends State<QRAddressWidget> {
  ...
  qrCode() {
    if (widget.paymentURI)
      GoRouter.of(context).push('/account/request');
    else {
      final qrUri =
