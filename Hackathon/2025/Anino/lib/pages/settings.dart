...
String resolveURL(CoinBase c, CoinSettings settings) {
  // Prefer selected built-in server index
  if (settings.lwd.index >= 0 && settings.lwd.index < c.lwd.length) {
    return c.lwd[settings.lwd.index].url;
  }
  // Next prefer a non-empty custom URL
  final custom = settings.lwd.customURL.trim();
  if (custom.isNotEmpty) return custom;
  // Fallback: choose the first built-in server if available
  if (c.lwd.isNotEmpty) {
    // Best-effort: set index so subsequent saves remember the choice
    settings.lwd.index = 0;
    return c.lwd.first.url;
  }
  // As last resort return empty to let caller handle error
  return '';
}
