<a href="https://github.com/zechub/zechub/edit/main/site/Using_Zcash/Custodial_Exchanges.md" target="_blank">
  <img src="https://img.shields.io/badge/Edit-blue" alt="Edit Page"/>
</a>

# <img src="/content-images/image-2024-02-03-173258092-a5440e5ee2.webp" alt="Alt Text" width="50"/>   Exchange con custodia

Ironwood è il pool schermato introdotto con l'aggiornamento della rete di luglio 2026. Gli exchange che gestiscono solo t-address trasparenti non lo utilizzano mai, quindi sono contrassegnati come non applicabili. Quando un exchange gestisce indirizzi schermati ma non ha dichiarato quale pool utilizza, la riga lo indica invece di fare supposizioni.

---

### [Backpack](https://backpack.exchange)

<a href="https://backpack.exchange">
    <img src="/content-images/_unavailable.svg" alt="Backpack Logo" width="200" height="100"/>
</a>

- Sito web: [Backpack](https://backpack.exchange)
- Coppie: ZEC/USDC | ZEC-PERP
- Supporta: Depositi e prelievi abilitati. Tipi di indirizzo non indicati
- Tempo di deposito: Non indicato
- Ironwood: Non indicato
___

### [Binance](https://binance.com)

<a href="https://binance.com">
    <img src="/content-images/_unavailable.svg" alt="Binance Logo" width="200" height="100"/>
</a>

- Sito web: [Binance](https://binance.com)
- Coppie: ALL/ZEC
- Supporta: Trasparente (deposito su indirizzo TEX)
- Tempo di deposito: 20 minuti
- Ironwood: Non applicabile, solo indirizzi trasparenti
___

### [BitcoinVN](https://bitcoinvn.io)

<a href="https://bitcoinvn.io">
    <img src="/content-images/_unavailable.svg" alt="BitcoinVN Logo" width="200" height="100"/>
</a>

- Sito web: [BitcoinVN](https://bitcoinvn.io)
- Coppie: BTC/ZEC, ETH/ZEC, USDT/ZEC, XMR/ZEC, LTC/ZEC, DASH/ZEC
- Supporta: Trasparente | Schermato (deposito e prelievo con z-address)
- Tempo di deposito: Immediato (swap non custodial)
- Ironwood: Non indicato
___

### [Bitfinex](https://bitfinex.com)

<a href="https://bitfinex.com">
    <img src="https://upload.wikimedia.org/wikipedia/en/4/41/Bitfinex_Logo_light.svg" alt="Bitfinex Logo" width="200" height="100"/>
</a>

- Sito web: [Bitfinex](https://bitfinex.com)
- Coppie: ALL/ZEC
- Supporta: Trasparente
- Tempo di deposito: 25 minuti 
- Ironwood: Non applicabile, solo indirizzi trasparenti
___

### [Coinbase](https://coinbase.com)

<a href="https://coinbase.com">
    <img src="/content-images/coinbase-8643659341.webp" alt="" width="200" height="100"/>
</a>

- Sito web: [Coinbase](https://coinbase.com)
- Coppie: ALL/ZEC
- Supporta: Trasparente
- Tempo di deposito: 150 minuti
- Ironwood: Non applicabile, solo indirizzi trasparenti
___

 ### [Gemini](https://gemini.com)

<a href="https://gemini.com">
    <img src="/content-images/Gemini-Symbol-ddd31ee98b.webp" alt="Gemini Custodian Exchange Logo" width="200" height="100"/>
</a>

- Sito web: [Gemini](https://gemini.com)
- Coppie: ALL/ZEC
- Supporta: Prelievi trasparenti, schermati e unificati, Orchard da novembre 2025
- Tempo di deposito: 50 minuti 
- Ironwood: Prelievi schermati supportati, pool non indicato
___

### [Huobi](https://huobi.com)

<a href="https://huobi.com">
    <img src="/content-images/huobi-global-logo-82DAA48E43-seeklogo.co-bc59e6f286.webp" alt="Huobi Logo" width="200" height="100"/>
</a>

- Sito web: [Huobi](https://huobi.com)
- Coppie: ALL/ZEC
- Supporta: Trasparente
- Tempo di deposito: 35 minuti 
- Ironwood: Non applicabile, solo indirizzi trasparenti
___

### [Kraken](https://kraken.com)

<a href="https://kraken.com">
    <img src="/content-images/kraken-logo-7407ab477f.webp" alt="Kraken Promotional" width="200" height="100"/>
</a>

- Sito web: [Kraken](https://kraken.com)
- Coppie: ALL/ZEC
- Supporta: Trasparente
- Tempo di deposito: 60 minuti
- Ironwood: Non applicabile, solo indirizzi trasparenti
___

### [KuCoin](https://kucoin.com)

<a href="https://kucoin.com">
    <img src="/content-images/kucoin-ee0c21f9fd.svg" alt="KuCoin Logo" width="200" height="100"/>
</a>

- Sito web: [KuCoin](https://kucoin.com)
- Coppie: ALL/ZEC
- Supporta: Trasparente
- Tempo di deposito: 20 minuti 
- Ironwood: Non applicabile, solo indirizzi trasparenti
___

## [OKEx](https://okex.com)

<a href="https://okex.com">
    <img src="https://upload.wikimedia.org/wikipedia/commons/8/89/Official_logo_of_OKEx.png" alt="OKEx Logo" width="200" height="100"/>
</a>

- Sito web: [OKEx](https://okex.com)
- Coppie: ALL/ZEC
- Supporta: Trasparente
- Tempo di deposito: 25 minuti 
- Ironwood: Non applicabile, solo indirizzi trasparenti
___

### [Robinhood](https://robinhood.com)

<a href="https://robinhood.com">
    <img src="/content-images/IMG-5725-7f964c1fdb.webp" alt="Robinhood Logo" width="200" height="100"/>
</a>

- Sito web: [Robinhood](https://robinhood.com)
- Coppie: USD/ZEC | EUR/ZEC
- Supporta: Prelievi solo verso t-address trasparenti. I depositi di ZEC non sono supportati
- Tempo di deposito: Depositi non supportati
- Ironwood: Non applicabile, solo indirizzi trasparenti
___
