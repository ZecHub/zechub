<a href="https://github.com/Zechub/zechub/edit/main/site/Zcash_Community/Cypherpunk_Zero_NFT.md" target="_blank">
  <img src="https://img.shields.io/badge/Edit-blue" alt="Edit Page"/>
</a>

![header](/content-images/205220798-c8e51db6-ddad-46ff-b760-401756-c9ba937b4e.webp)


# Cypherpunk Zero

Cypherpunk Zero è una serie narrativa incentrata su Zero, una giovane hacker cypherpunk e combattente per la libertà. Zero vive attualmente in un incubo distopico e usa il codice per combattere contro i guardiani centralizzati che incatenano la società. La storia è stata ispirata da Zcash e dalla crittografia Halo. L'opera creativa è uno sforzo collaborativo tra ECC, Stranger World, Mighty Jaxx e alcuni partner selezionati dell'ecosistema. L'opera creativa si è concentrata soprattutto su una [serie di NFT](https://opensea.io/collection/cypherpunk-zero), ma sono stati rilasciati anche un [fumetto prologo](https://halo.electriccoin.co/#view-prologue) e un [giocattolo da collezione](https://mightyjaxx.com/products/cypherpunk-zero). La comunità degli NFT, attiva soprattutto su Twitter, ha persino creato un'[organizzazione decentralizzata (DAO)](https://twitter.com/CypherpunkDAO) per organizzare e completare progetti a sostegno della campagna e della più ampia comunità Zcash.

## Anti-roadmap Roadmap

Abbiamo visto troppi progetti promettere troppo e mantenere troppo poco riguardo alle loro roadmap NFT. Per questo motivo, non esiste una roadmap formale. Fidati: abbiamo in cantiere alcuni vantaggi esclusivi straordinari per i possessori di NFT, alcuni attesi, altri inaspettati. Allacciate le cinture e venite con noi per questo viaggio.

## Governance

[Snapshot](https://vote.cypherpunkzero.com/)

[Guild](https://guild.xyz/cypherpunkzerodao)

[Commonwealth](https://commonwealth.im/cypherpunk-zero)

## Chi è Stranger Wolf?

<div className="my-8 w-full aspect-video max-w-3xl mx-auto rounded-2xl overflow-hidden shadow-lg bg-black">
  <iframe
    className="w-full h-full"
    src="https://www.youtube.com/embed/KF-VwIOYDlE"
    title="Strange Wolf - Awake"
    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
    allowFullScreen
    loading="lazy"
  />
</div>


## Su quale chain si trovano gli NFT?

[Ethereum](https://etherscan.io/address/0x3e86d6cf041b719c575f57050697c115f0a53758)

## Quali sono le allocazioni dei token?

*Offerta totale*: 10.000 NFT generativi unici sulla Ethereum Mainnet
Il 4% dell'offerta (i primi 400 NFT) è stato distribuito tramite airdrop ai primi sostenitori del progetto che avevano acquistato il giocattolo Zcash X Mighty Jaxx: Cypherpunk Zero Genesis Edition Toy 59.

Il 72% dell'offerta (7.200 NFT) sarà rilasciato al pubblico usando un innovativo meccanismo di whitelisting che richiede l'uso di un wallet Zcash che supporti i memo cifrati (maggiori dettagli più in basso).

Il 24% dell'offerta (2.400 NFT) sarà conservato da ECC nella Cypherpunk Reserve, per essere usato in future iniziative che facciano progredire Zcash e la privacy nello spazio Web3.


# Whitelist di Cypherpunk Zero v2 

Sì, la collezione originale è stata hackerata, il futuro è ancora da scrivere.

## Risorse

[Sito web di Cypherpunk Zero](https://halo.electriccoin.co/)

[Twitter di Cypherpunk Zero](https://twitter.com/cypherpunkZero)

[Collezione Opensea di Cypherpunk Zero](https://opensea.io/collection/cypherpunk-zero)

[Twitter della DAO di Cypherpunk Zero](https://twitter.com/CypherpunkDAO)

[Discord della DAO di Cypherpunk Zero](https://discord.com/invite/sjfgXys4Jf)

[Mega Thread sugli NFT di Cypherpunk Zero](https://forum.zcashcommunity.com/t/cypherpunk-zero-nft-megathread/41502?u=dismad)
