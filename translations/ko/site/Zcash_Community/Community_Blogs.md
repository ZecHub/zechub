[![페이지 편집](https://img.shields.io/badge/Edit-blue)](https://github.com/Zechub/zechub/edit/main/site/Zcash_Community/Community_Blogs.md)

# 커뮤니티 블로그

커뮤니티 구성원들은 Zcash, 프라이버시, 암호화폐 및 관련 주제를 다루는 훌륭한 블로그들을 운영하고 있습니다.

활발하게 운영되고 있는 몇몇 블로그는 다음과 같습니다:

| 블로그 / 작성자              | 설명                                              | 링크 |
|----------------------------|----------------------------------------------------------|------|
| How Zcash works            | Zcash의 작동 모델을 구축하기                           | [방문 ->](https://community.pgpz.org/zec-shelf/how-zcash-works.html#top) |
| Max Desalle                | Mastering Zcash                                          | [방문 ->](https://maxdesalle.com/mastering-zcash/) |
| Mike Rosulek               | 암호학의 즐거움                                  | [방문 ->](https://joyofcryptography.com/) |
| James Katz                 | Zcash와 프라이버시에 대한 개인적인 글과 생각      | [방문 ->](https://free2z.cash/James_Katz/) |
| Thumbs' Update             | 정기적인 생태계 업데이트와 인사이트                   | [방문 ->](https://thumbsup.substack.com) |
| roomatemusing              | 단상과 커뮤니티 콘텐츠                            | [방문 ->](https://free2z.cash/roommatemusing) |
| NerdBank Blog              | Zcash 개발 및 도구에 초점을 맞춘 기술 블로그    | [방문 ->](https://blog.nerdbank.net/) |
| Thor Likes                 | Zcash 관련 뉴스, 의견 및 해설                  | [방문 ->](https://www.thorlikes.com/) |
| ZecMec                     | Medium에 게시된 Zcash 중심 기사         | [방문 ->](https://zecmec21.medium.com/) |
| Ian Sagstetter             | 심층 기사와 뉴스레터                         | [방문 ->](https://iansagstetter.substack.com/) |
| Naomi Brockwell (NBTV)     | 프라이버시에 관한 High-profile 인터뷰와 콘텐츠           | [방문 ->](https://naomibrockwell.com/highprofileinterviews) |
| Sqribbles                  | 창의적이고 커뮤니티 주도의 Zcash 콘텐츠              | [방문 ->](https://free2z.cash/sqribbles) |
| Str4d                      | Zcash 핵심 개발자의 기술 관련 글             | [방문 ->](https://words.str4d.xyz/) |
| CipherScan Weekly          | Zcash 블록체인 분석을 다루는 주간 블로그 게시물    | [방문 ->](https://cipherscan.app/) |
| Zcash Shielded News        | 프라이버시 및 생태계 관련 주간 뉴스 다이제스트                 | [방문 ->](https://zechub.substack.com/) |
| ZecHub DAO Blog            | ZecHub DAO 커뮤니티의 기사와 교육 콘텐츠 | [방문 ->](https://zechub.wiki) |

---

다음은 커뮤니티가 제출한 블로그입니다. 본문에 자신의 블로그 글을 소개하고 싶거나, ZecHub에서 특별히 다루고 싶다면 세부 정보를 포함한 Pull Request를 생성해 주세요!
