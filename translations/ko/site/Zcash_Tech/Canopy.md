...
| 용어 | 쉬운 의미 |
|---|---|
| Founders reward | 초기 Zcash 개발 자금을 지원했던 원래의 자금 조달 모델로, 첫 반감기와 함께 종료되도록 예정되어 있었음 |
| Development Fund | Canopy가 개발 자금으로 보내도록 한 각 블록 보상의 20퍼센트 지분으로, 두 번째 반감기까지 운영됨 |
| Block reward (subsidy) | 각 블록이 채굴될 때 새로 생성되어 지급되는 ZEC |
| Halving | 블록 보상이 절반으로 줄어드는 예정된 이벤트 |
| Funding stream | 블록 보조금의 일부를 정해진 수령 주소로 보내는 합의 메커니즘 (ZIP 207) |
| Sprout pool | Zcash의 원래 실드 풀로, Canopy 이후 새로운 가치의 유입이 중단됨 |
