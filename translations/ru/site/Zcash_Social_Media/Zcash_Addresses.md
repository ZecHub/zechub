...
Этот тред изначально ссылался на **Ywallet** из-за плана транзакции, который он показывал перед отправкой. Ywallet больше не поддерживается и не будет обновлён для Ironwood, поэтому он больше не может следовать за блокчейном. Вместо этого выберите поддерживаемый кошелёк на странице [Кошельки](https://zechub.wiki/wallets) и отдайте предпочтение тому, который сообщает, что транзакция раскроет до её отправки.
