...
Zcash (ZEC) ist eine auf Privatsphäre ausgerichtete Kryptowährung, die den Equihash-Proof-of-Work-Algorithmus für das Mining verwendet. Beim Mining von Zcash wird Rechenleistung eingesetzt, um komplexe mathematische Probleme zu lösen, Transaktionen zu validieren und das Netzwerk im Austausch gegen ZEC-Belohnungen zu sichern. Aufgrund der hohen Netzwerkschwierigkeit wird Solo-Mining für die meisten Nutzer nicht empfohlen. Der Beitritt zu einem Mining-Pool ist der beste Weg, um durch die Bündelung Ihrer Hash-Leistung mit anderen konstante Belohnungen zu erzielen.

Dieser Leitfaden konzentriert sich auf das Mining von Zcash mit eigener Hardware (z. B. einem Heim-PC mit GPUs oder ASICs der Einstiegsklasse). Beachten Sie, dass GPUs zwar weiterhin Zcash minen können, ASICs im Jahr 2026 aufgrund der Netzwerkschwierigkeit jedoch deutlich effizienter und profitabler sind. Prüfen Sie die aktuelle Rentabilität immer mit Tools wie WhatToMine.com, da Faktoren wie Stromkosten, Hardwarepreise und der ZEC-Wert die Wirtschaftlichkeit beeinflussen. Mining ist nicht für jeden profitabel; informieren Sie sich über lokale Vorschriften und Energietarife (angestrebt < 0,08 $/kWh).


## Anforderungen

### Hardware
- **GPU-Mining (für Anfänger empfohlenes persönliches Setup):**
  - NVIDIA- oder AMD-GPUs mit mindestens 4 GB VRAM (z. B. NVIDIA GTX 1070, RTX 3060; AMD RX 580 oder besser).
  - Ein kompatibles Mainboard, ein ausreichend starkes Netzteil (mindestens 750 W für mehrere GPUs) und gute Kühlung, um Überhitzung zu vermeiden.
  - Multi-GPU-Rigs sind für bessere Hash-Raten üblich (z. B. können 6x GPUs 1–2 kSol/s erreichen).
...
- **Allgemein:** Stabile Internetverbindung, ein Computer für Einrichtung/Überwachung. ASICs dominieren das Netzwerk (~13 GSol/s Gesamthashrate im Jahr 2026), wodurch GPU-Mining weniger wettbewerbsfähig ist, für Hobbyisten aber weiterhin möglich bleibt.

### Software
- **Betriebssystem:** Windows 10/11, Linux (Ubuntu wird für Stabilität empfohlen).
- **Mining-Software:**
  - Für GPUs: lolMiner (unterstützt AMD/NVIDIA), GMiner oder miniZ (NVIDIA-fokussiert). Download von den offiziellen GitHub-Repos (z. B. github.com/Lolliedieb/lolMiner-releases).
  - Für ASICs: Verwenden Sie die integrierte Firmware/das Dashboard des Herstellers (z. B. die Weboberfläche von Bitmain).
- **Wallet:** Eine Zcash-Wallet zum Empfangen von Auszahlungen. Empfohlen:
  - Shielded (privat): Zashi Wallet, Zingo (Mobile/Desktop) YWallet (mobile/desktop).
  - Transparent (einfacher, aber weniger privat): Edge Wallet, Zecwallet Lite.
  - Download von [Wallets](https://zechub.wiki/wallets). Erzeugen Sie eine shielded Adresse (beginnt mit „zs“) für Privatsphäre, falls der Pool sie unterstützt.
...
- Für GPUs:
  ...
  2. Übertakten Sie nur bei entsprechender Erfahrung (verwenden Sie MSI Afterburner für Stabilität; angestrebt +100–200 Core Clock, -500 Speicher für Effizienz).
- Für ASICs:
...
**Warnung:** Sorgen Sie für ausreichende Belüftung; Mining erzeugt Hitze. Beginnen Sie klein, um zu testen.

### Schritt 3: Wählen Sie einen Mining-Pool und treten Sie ihm bei
Mining-Pools verteilen Arbeit und teilen Belohnungen basierend auf Ihrer beigetragenen Hashrate. Wählen Sie nach Gebühren (0–2 %), Auszahlungslimit (0,01–0,1 ZEC), Standort (niedriger Ping) und Zuverlässigkeit aus.
...
- **Sovright (mining.sovright.com)**: Ein Zcash-Pool auf Basis von Stratum V2, der derzeit als öffentliches Testnet läuft. Noch keine Live-ZEC-Auszahlungen, daher eher als Möglichkeit zum Testen Ihres Setups als als Einnahmequelle betrachten. Details finden Sie im eigenen Abschnitt weiter unten.
- Andere: Kryptex Pool, Luxor (prüfen Sie poolwatch.io/coin/zcash für Echtzeitstatistiken).

1. Besuchen Sie die Website des Pools und erstellen Sie ein Konto (E-Mail oder bei einigen wie 2Miners ohne Registrierung).
2. Fügen Sie in den Einstellungen Ihre Zcash-Wallet-Adresse für Auszahlungen hinzu.
3. Notieren Sie sich den Stratum-Server des Pools (z. B. zec.2miners.com:1010) und den Port.
...
- Für ASICs (Beispiel: Bitmain Antminer):
  ...
  4. Speichern und den Miner neu starten.
...
**Test:** 10–15 Minuten laufen lassen; Konsole auf akzeptierte Shares und Hashrate prüfen.
...
2. Überwachung über:
   ...
   - Tools: Verwenden Sie HiveOS oder SimpleMining OS für die Remote-Verwaltung von Rigs.
3. Auszahlungen: Die meisten Pools zahlen automatisch aus, wenn Sie das Minimum erreichen (z. B. 0,05 ZEC). Prüfen Sie die Pool-Regeln.
...
Sovright (sovright.com) betreibt einen Stratum-V2-Mining-Pool und ein separates Block-Relay-Netzwerk. Diese haben unterschiedliche Aufgaben und werden daher unten getrennt behandelt.

### Mining-Pool (mining.sovright.com)

Der Pool von Sovright läuft auf einem öffentlichen Zcash-Testnet (NU6, Stratum V2), nicht auf dem Mainnet. Das Testnet zahlt keine echten ZEC aus. Verwenden Sie es, um Ihre Miner-Konfiguration zu testen, nicht um Erträge zu erzielen.

- Zum Start ist kein Konto erforderlich. Richten Sie einen CPU- oder ASIC-Equihash-Miner auf den Pool, und Ihre Shares erscheinen auf einem Live-Dashboard.
- Sovright veröffentlicht außerdem einen Open-Source-Stratum-V2-Proxy für Miner, die ihre eigenen Block-Templates wählen möchten, anstatt einfach die Jobs des Pools zu übernehmen:
  ```
  ...
  ```
  Richten Sie Ihren Miner auf den Proxy statt direkt auf den Pool:
  ```
  stratum+tcp://<your-proxy-ip>:3334
  ```
  und verwenden Sie dabei einen Worker-Namen wie `yourname.rig1`.
- Auf der Transparenzseite von Sovright wird eine „include all“-Richtlinie für shielded Transaktionen angegeben, im Gegensatz zu einigen Pools, die diese herausfiltern. Jeder Block erhält eine signierte Bestätigung, sodass die Richtlinie unabhängig überprüft werden kann.
- Erstellen Sie ein Konto bei mining.sovright.com (Google- oder E-Mail-Anmeldung), um Ihre eigenen Worker statt der Beispieldaten im Dashboard zu verfolgen.

### Relay-Netzwerk (relay.sovright.com)

Sovright betreibt außerdem ein öffentliches Block-Relay-Netzwerk auf dem Zcash-Mainnet. Wenn ein Pool einen Block findet, bestimmt die Geschwindigkeit, mit der dieser Block den Rest des Netzwerks erreicht, wie oft er verwaist wird, also das Weiterleitungsrennen verliert und dadurch die Belohnung verloren geht. Das Relay leitet Blöcke über vier Regionen mithilfe von Compact-Block-Relay mit Forward Error Correction weiter.

Das öffentliche Dashboard zeigt die Wirkung live: Mit dem Relay verbundene Regionen sehen neue Blöcke in deutlich weniger als der Hälfte der Zeit, die gewöhnliches Peer-to-Peer-Gossip benötigt, und das Dashboard verfolgt die aktuelle Orphan-Rate des Netzwerks.
...
- **Privatsphäre:** Verwenden Sie shielded Pools, falls verfügbar; vermeiden Sie die Wiederverwendung von Adressen.
...
- **Alternativen:** Wenn es nicht profitabel ist, ziehen Sie Cloud-Mining oder Staking anderer Coins in Betracht.
- **Umwelthinweis:** Mining verbraucht Energie; verwenden Sie nach Möglichkeit erneuerbare Energiequellen.
- **Updates:** Zcash kann sich weiterentwickeln (z. B. möglicher Wechsel zu PoS); prüfen Sie z.cash auf Neuigkeiten.
