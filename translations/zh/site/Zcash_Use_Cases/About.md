# 在现实世界中使用 Zcash

Zcash 不只是停留在理论层面的隐私——它关乎**实用的、日常的金融自由**。

本节将向你**准确展示如何在现实生活场景中使用 Zcash**，并提供分步指南与最佳实践。

## 你将学到什么

- 如何保护你的财务隐私
- 如何避免暴露数据的常见错误
- 如何在现实世界场景中应用 Zcash

## 如何使用这些指南

每份实战指南都具备以下特点：
- 简短（5-7 分钟）
- 分步说明
- 聚焦现实世界


如果你是新手，请从这里开始：[什么是 Zcash](/start-here/what-is-zec-and-zcash)


## 推荐路径

按照以下分步学习路线，掌握现实世界中的 Zcash 使用方法：

### [私密地接收捐款](/zcash-use-cases/receive-donations-privately)
了解如何在不暴露身份或财务历史的情况下接收资金。


### [在不关联身份的情况下汇款](/zcash-use-cases/send-money-without-linking-identity) 
在发送资金时，避免暴露你的钱包、身份或交易图谱。


###  [自由职业者隐私配置](/zcash-use-cases/freelance-privacy-setup)
以 Zcash 收款，同时保护你的客户信息和收入隐私。


### [作为商家接受付款](/zcash-use-cases/accept-payments-as-a-merchant)  
使用屏蔽地址接受付款，避免暴露客户交易数据


###  [运行私密的社区金库](/zcash-use-cases/private-community-treasury)
使用屏蔽地址持有共享资金，并限制余额和交易的可见性

### [记者隐私配置](/zcash-use-cases/journalist-privacy-setup)   
对所有交易使用屏蔽地址，并通过避免可追踪付款来保护消息来源。谨慎使用备忘录以实现安全通信

<br/>
