...
| 术语 | 通俗含义 |
|---|---|
| Network upgrade (NU) | 对 Zcash 共识规则进行协调一致的变更，并在指定区块高度激活 |
| Consensus branch id | 用于标识当前这套共识规则的简短标识符 |
| Activation height | 网络升级的新规则开始生效的区块 |
| Replay protection | 阻止一笔在某条链上有效的交易被拿到另一条链上复用的规则 |
| Mempool | 已广播但尚未被打包进区块的交易池 |
| Transaction expiry | 一个过期区块高度，超过之后，未被打包的交易将被丢弃 |
