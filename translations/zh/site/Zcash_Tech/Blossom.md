...
| 术语 | 通俗含义 |
|---|---|
| Block target spacing | 网络希望从一个区块到下一个区块之间经过的时间 |
| Block reward | 每个区块被挖出时新创建并支付的 ZEC |
| Halving interval | 每次区块奖励减半之间经过多少个区块 |
| Consensus branch id | 用于标记某个节点或交易正在遵循哪一套网络规则的标签 |
| Bilateral consensus change | 每个节点都必须采用、才能继续留在网络中的规则变更 |
| Network upgrade (NU) | 对 Zcash 共识规则进行的一次协调性变更，在设定的区块高度激活 |
