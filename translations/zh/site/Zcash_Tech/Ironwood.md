...
> Ironwood 已于 2026 年 7 月 28 日 UTC 在 Zcash 主网的区块 3,428,143 激活，并自此一直运行。
...
| 术语 | 通俗含义 |
|---|---|
| Shielded pool | 一组其金额和所有者通过零知识密码学隐藏起来的资金 |
| Soundness bug | 一种缺陷，它会让无效交易像有效交易一样通过证明检查 |
| Turnstile | 一个公开检查点，用于统计在不同池之间流动的价值，从而让供应量保持可审计 |
| Spend-only | 一个你可以从中支出、但不能再向其中加入新价值的池 |
| Network upgrade (NU) | 对 Zcash 共识规则进行的一次协调变更，在设定的区块高度激活 |
| Quantum-recoverable note | 一种 note 格式，其设计目标是如果量子计算机有朝一日攻破当今密码学，资金仍可被恢复 |
