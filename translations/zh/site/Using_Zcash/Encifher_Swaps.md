...
###  步骤 2：包装你的代币  
进入 **Wrap** 部分。选择 **SOL** 或 **USDC**，输入数量并确认。应用会锁定你的资产，并发行其对应的**加密版本（eSOL 或 eUSDC）**。为了提升隐私性，建议包装略多于你计划兑换的数量——这样可以避免一一对应的可追踪性。  
...
###  步骤 3：准备你的 Zodl 钱包  
下载由 ZODL 维护的 [**Zodl**](https://zodl.com) Zcash 钱包。从接收标签页复制你的 **Unified Address**——它同时支持透明和屏蔽的 ZEC。继续之前，请先安全保存好你的助记词。
...
###  步骤 4：私密兑换  
回到 **encrypt.trade**，进入 **Swap**。选择 **eSOL/eUSDC -> ZEC**，粘贴你的 Zodl 地址，检查详情并确认。
...
**NEAR Intents** 引擎会自动处理跨链路由——在几秒内将 **ZEC** 直接发送到你的 Zodl 钱包。
...
###  步骤 5：屏蔽并保持私密  
收到后，使用 ZODL 的 **Shield** 选项将你的 ZEC 转入屏蔽池，以获得最大程度的隐私保护。务必始终验证链接，避免重复使用地址，并先用小额进行测试。

---

通过结合 **Solana 的速度**、**Zcash 的隐私性** 和 **NEAR Intents 的自动化能力**，**encrypt.trade** 重新定义了私密加密货币兑换的可能性——快速、顺畅且真正保密。
