...
- Recevez des paiements via des adresses blindées
- Séparez les identités par client (si nécessaire)
- Utilisez des mémos pour le suivi interne
- Gardez vos revenus privés
...
- Professionnels soucieux de leur vie privée
...
- Masquer vos revenus
- Protéger les relations avec vos clients
...
Générez une adresse blindée (z).

En option :
- Utilisez des adresses différentes pour différents clients


### Étape 2 : Partager l’adresse avec le client

Envoyez directement votre adresse z à votre client.

Évitez :
- De la publier publiquement si ce n’est pas nécessaire

### Étape 3 : Utiliser des mémos pour le suivi

Lors de la réception de paiements :
- Demandez aux clients d’inclure un mémo (par ex., l’ID de la facture)

Cela vous aide à :
- Suivre les paiements sans exposer publiquement les données


### Étape 4 : Maintenir la séparation des identités

Si nécessaire :
- Utilisez des portefeuilles ou des adresses séparés pour chaque client
- Évitez de relier les identités entre elles
...
- Associer publiquement le portefeuille à votre identité  
- Ne pas suivre les paiements (ce qui entraîne de la confusion)
...
Vous pouvez :
- Recevoir des paiements en privé
- Protéger les données de vos revenus
- Conserver une flexibilité professionnelle
...
Vous comprenez désormais comment recevoir des revenus en privé.
