...
> **Historique. Ce tutoriel ne fonctionne plus.**
>
> Chaque étape ci-dessous dépend de zcashd, qui a atteint son arrêt automatique de fin de support le 18 juillet 2026. Les sept scripts fournis avec cette page le pilotent via `zcash-cli`, donc aucun d’eux ne peut atteindre un nœud en cours d’exécution aujourd’hui.
>
> Ces scripts ne peuvent pas être portés mécaniquement. Ils sont construits sur les RPC de transactions brutes et de wallet (`createrawtransaction`, `signrawtransaction`, `createmultisig`, `dumpprivkey`) que zcashd a dépréciés avant l’arrêt ; Zallet les remplace par de nouvelles méthodes qui fonctionnent sur des PCZT plutôt que sur du hexadécimal de transaction brute, et reste en bêta avec de nombreuses méthodes zcashd qui n’ont pas encore été portées.
>
> Pour la garde multipartite sur Zcash aujourd’hui, consultez [FROST & garde à seuil](/zcash-tech/frost-threshold-custody), qui comprend une comparaison directe avec le multisig transparent, ainsi que la [démo FROST Ywallet](/guides/frostdemo/ywallet-frost-demo) fonctionnelle. Pour migrer un nœud existant depuis zcashd, consultez le [guide de migration vers Zebra et Zallet](/guides/migration-guide-zcashd-to-zebrad-zallet).
>
> Cette page est conservée comme archive historique du processus de multisig transparent.
...
## Créer 2x adresses t3 MultiSig (2 sur 3)
...
`./createMultiSig.sh pubk4 pubk5 pubk6`      # 2e t3 pour l’adresse de monnaie restante. 
...
Utilisez n’importe quel wallet/faucet pour alimenter l’adresse
...
Si vous avez accès à toutes vos clés privées, vous pouvez les utiliser toutes en une fois pour gagner du temps,
mais dans la plupart des exemples réels, la signature sera effectuée par des personnes réparties dans le monde entier ; chacun des participants requis devra donc signer,
...
Cette commande s’est arrêtée avec zcashd et ne renvoie rien aujourd’hui ; elle est enregistrée ici uniquement pour montrer comment la démo obtenait ses clés.
