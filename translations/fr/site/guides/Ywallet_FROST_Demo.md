# Démo FROST avec Ywallet

> **Ywallet n’est plus maintenu.** Son développeur a confirmé qu’il ne sera pas mis à jour pour Ironwood (NU6.3), il ne peut donc plus suivre la chaîne et les étapes ci-dessous ne peuvent pas être réalisées sur le mainnet. Cette page est conservée à titre de référence. Zkool, du même développeur, est son successeur maintenu et prend en charge le multisig FROST.
...
## Demander à chaque participant de signer cette transaction
...
Dans la fenêtre du coordinateur, copiez la signature de groupe produite et collez-la dans la fenêtre de signature FROST.
...
Si tout fonctionne, vous obtiendrez un identifiant de transaction :)
