...
ZecWeekly est une newsletter envoyée chaque dimanche matin. Elle inclut toutes les actualités survenues au cours de la semaine dans l’écosystème Zcash. Les actualités sont sélectionnées chaque semaine par des membres de la communauté et tous les liens pertinents sont ajoutés à la newsletter. Veuillez vous abonner à la newsletter [ici](https://zechub.substack.com/).

## Contribuer

Les contributions à la newsletter fonctionnent mieux lorsqu’un contributeur prépare l’édition pour la bonne semaine, suit le fil de discussion actuel sur la prime ou la coordination, et soumet la pull request une fois les liens hebdomadaires prêts. Veuillez ne pas soumettre une future édition avant que ZecHub ait publié ou confirmé la date de cette édition. Les pull requests soumises trop tôt omettent souvent des mises à jour de fin de semaine, entrent en conflit avec un curateur désigné ou utilisent la mauvaise échéance.
...
Si vous débutez avec GitHub, suivez ce processus :
...
4. Assurez-vous que votre pull request ciblera `ZecHub/zechub` comme dépôt de base et `main` comme branche de base.
...
- Respectez le format de nom de fichier demandé par l’issue ou utilisé par les éditions récemment acceptées.
...
- Commencez à collecter les liens après la publication de l’issue ou de la tâche de la newsletter actuelle.
...
- Ne soumettez pas la newsletter d’une future semaine avant que la tâche pour cette date existe ou avant que ZecHub confirme que vous devez la préparer.

Si une issue indique une date de soumission précise, respectez cette date. En cas de conflit entre cette page et une issue actuelle, suivez l’issue actuelle.
...
Après l’ouverture de la pull request, surveillez les commentaires de révision. Si ZecHub demande des modifications, mettez à jour la même branche au lieu d’ouvrir une seconde pull request pour la même édition.

### Exemples réels

Utilisez ces pull requests de newsletters fusionnées comme exemples de soumissions acceptées :
...
Lorsque vous comparez votre travail avec un exemple, concentrez-vous sur l’emplacement du fichier, le format du titre, l’ordre des sections, les descriptions des liens et la vérification que la pull request renvoie à la bonne tâche.
...
- Laisser des liens cassés, des liens dupliqués ou du texte d’espace réservé du modèle.
- Ouvrir une nouvelle pull request après des commentaires de révision au lieu de mettre à jour la branche d’origine.

### Liste de vérification finale

Avant de demander une révision, vérifiez que :
...
- Le corps de la pull request renvoie à la bonne issue.
