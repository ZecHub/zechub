<a href="https://github.com/zechub/zechub/edit/main/site/Using_Zcash/Custodial_Exchanges.md" target="_blank">
  <img src="https://img.shields.io/badge/Edit-blue" alt="Edit Page"/>
</a>

# <img src="/content-images/image-2024-02-03-173258092-a5440e5ee2.webp" alt="Alt Text" width="50"/>   Plateformes d'échange custodiales

Ironwood est le pool protégé arrivé avec la mise à niveau du réseau de juillet 2026. Les plateformes d'échange qui ne traitent que les t-adresses transparentes n'y ont jamais accès ; elles sont donc indiquées comme non applicables. Lorsqu'une plateforme prend en charge les adresses protégées mais n'a pas précisé quel pool elle utilise, la ligne l'indique plutôt que de deviner.

---

### [Backpack](https://backpack.exchange)

<a href="https://backpack.exchange">
    <img src="/content-images/_unavailable.svg" alt="Backpack Logo" width="200" height="100"/>
</a>

- Site web : [Backpack](https://backpack.exchange)
- Paires : ZEC/USDC | ZEC-PERP
- Prend en charge : dépôts et retraits activés. Types d'adresses non indiqués
- Délai de dépôt : non indiqué
- Ironwood : non indiqué
___

### [Binance](https://binance.com)

<a href="https://binance.com">
    <img src="/content-images/_unavailable.svg" alt="Binance Logo" width="200" height="100"/>
</a>

- Site web : [Binance](https://binance.com)
- Paires : ALL/ZEC
- Prend en charge : transparent (dépôt à une adresse TEX)
- Délai de dépôt : 20 minutes
- Ironwood : non applicable, adresses transparentes uniquement
___

### [BitcoinVN](https://bitcoinvn.io)

<a href="https://bitcoinvn.io">
    <img src="/content-images/_unavailable.svg" alt="BitcoinVN Logo" width="200" height="100"/>
</a>

- Site web : [BitcoinVN](https://bitcoinvn.io)
- Paires : BTC/ZEC, ETH/ZEC, USDT/ZEC, XMR/ZEC, LTC/ZEC, DASH/ZEC
- Prend en charge : transparent | protégé (dépôt et retrait à une z-adresse)
- Délai de dépôt : instantané (swap non custodial)
- Ironwood : non indiqué
___

### [Bitfinex](https://bitfinex.com)

<a href="https://bitfinex.com">
    <img src="https://upload.wikimedia.org/wikipedia/en/4/41/Bitfinex_Logo_light.svg" alt="Bitfinex Logo" width="200" height="100"/>
</a>

- Site web : [Bitfinex](https://bitfinex.com)
- Paires : ALL/ZEC
- Prend en charge : transparent
- Délai de dépôt : 25 minutes 
- Ironwood : non applicable, adresses transparentes uniquement
___

### [Coinbase](https://coinbase.com)

<a href="https://coinbase.com">
    <img src="/content-images/coinbase-8643659341.webp" alt="" width="200" height="100"/>
</a>

- Site web : [Coinbase](https://coinbase.com)
- Paires : ALL/ZEC
- Prend en charge : transparent
- Délai de dépôt : 150 minutes
- Ironwood : non applicable, adresses transparentes uniquement
___

 ### [Gemini](https://gemini.com)

<a href="https://gemini.com">
    <img src="/content-images/Gemini-Symbol-ddd31ee98b.webp" alt="Gemini Custodian Exchange Logo" width="200" height="100"/>
</a>

- Site web : [Gemini](https://gemini.com)
- Paires : ALL/ZEC
- Prend en charge : retraits transparents, protégés et unifiés, Orchard depuis novembre 2025
- Délai de dépôt : 50 minutes 
- Ironwood : retraits protégés pris en charge, pool non indiqué
___

### [Huobi](https://huobi.com)

<a href="https://huobi.com">
    <img src="/content-images/huobi-global-logo-82DAA48E43-seeklogo.co-bc59e6f286.webp" alt="Huobi Logo" width="200" height="100"/>
</a>

- Site web : [Huobi](https://huobi.com)
- Paires : ALL/ZEC
- Prend en charge : transparent
- Délai de dépôt : 35 minutes 
- Ironwood : non applicable, adresses transparentes uniquement
___

### [Kraken](https://kraken.com)

<a href="https://kraken.com">
    <img src="/content-images/kraken-logo-7407ab477f.webp" alt="Kraken Promotional" width="200" height="100"/>
</a>

- Site web : [Kraken](https://kraken.com)
- Paires : ALL/ZEC
- Prend en charge : transparent
- Délai de dépôt : 60 minutes
- Ironwood : non applicable, adresses transparentes uniquement
___

### [KuCoin](https://kucoin.com)

<a href="https://kucoin.com">
    <img src="/content-images/kucoin-ee0c21f9fd.svg" alt="KuCoin Logo" width="200" height="100"/>
</a>

- Site web : [KuCoin](https://kucoin.com)
- Paires : ALL/ZEC
- Prend en charge : transparent
- Délai de dépôt : 20 minutes 
- Ironwood : non applicable, adresses transparentes uniquement
___

## [OKX (anciennement OKEx)](https://www.okx.com)

<a href="https://www.okx.com">
    <img src="https://upload.wikimedia.org/wikipedia/commons/8/89/Official_logo_of_OKEx.png" alt="Logo OKX (affiché sous son ancienne marque OKEx)" width="200" height="100"/>
</a>

- Site web : [OKX](https://www.okx.com)
- Paires : ALL/ZEC
- Prend en charge : transparent
- Délai de dépôt : 25 minutes 
- Ironwood : non applicable, adresses transparentes uniquement
___

### [Robinhood](https://robinhood.com)

<a href="https://robinhood.com">
    <img src="/content-images/IMG-5725-7f964c1fdb.webp" alt="Robinhood Logo" width="200" height="100"/>
</a>

- Site web : [Robinhood](https://robinhood.com)
- Paires : USD/ZEC | EUR/ZEC
- Prend en charge : retraits vers des t-adresses transparentes uniquement. Les dépôts ZEC ne sont pas pris en charge
- Délai de dépôt : dépôts non pris en charge
- Ironwood : non applicable, adresses transparentes uniquement
___
