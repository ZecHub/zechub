...
* Nnipa dodow no ara nam sika kotoku a emu yɛ hare so de Zcash di dwuma, a ɛntwe blockchain no nyinaa. Mmom no, ɛne server bi a ayɛ saa adwuma no dedaw kasa.
* Software asinasin mmienu som sika kotokuo a emu yɛ hare nnɛ: **lightwalletd**, mfitiaseɛ dwumadie a wɔkyerɛw no Go, ne **Zaino**, indexer foforɔ a wɔakyerɛw wɔ Rust mu.
* Wo safe no mfi wo mfiri no mu da, na server no ntumi nsɛe wo sika anaasɛ nkenkan sika dodow ne memos a ɛwɔ nnwuma a wɔabɔ ho ban koraa mu.
* Nea server no wɔ beae pa sɛ wubesua ne wo IP address ne bere a wo dwumadi — shielded transactions bɔ nea ɛkɔ blockchain no so ho ban, ɛnyɛ wo nkitahodi ne server no.
* Tor yi IP nkyerɛkyerɛmu no fi hɔ; ɛwɔ sika kotoku a wɔde asi so mu `zcash_client_backend`, na wɔ ZODL mu no ɛyɛ nhyehyɛe a ɛwɔ Advanced Settings mu.
* Wubetumi asesa server a wo sika kotoku no de di dwuma, anaasɛ wobɛma w’ankasa de ayɛ adwuma — lightwalletd ne Zaino nyinaa yɛ open source.
...
Ɛsom compact blocks. Sɛ́ anka ɛde blocks mũ no nyinaa bɛmena no, ɛde kratasin ketewaa bi a nea sika kotoku hia na ama wɔahu sika a wɔatua nkutoo na ɛwɔ mu kɔ ne address a wɔabɔ ho ban no so, ahu sika a wɔsɛee no wɔ ne nsɛm a wɔakyerɛw no so, na ama n’adansefo ayɛ foforo.
...
Wo sika kotoku no da so ara yɛ kokoam adwuma no wɔ mpɔtam hɔ. Ɛkura wo nsafe, ɛsɔ blocks hwɛ de hwehwɛ wo nsɛm a woakyerɛw, na ɛkyekyere na ɛde ne nsa hyɛ nnwuma ase wɔ wo mfiri no so.
...
Fi ase fi nea wɔabɔ ho ban no so. Wɔ ɔtamfo biara a ɔwɔ model no mu, a obi a wasɛe server no ka ho, "entumi nsua ɔdefo no cryptographic key material biara (spending keys, viewing keys, seed phrase, ne nea ɛkeka ho)", entumi nwia wo sika, na entumi mma womfa sika a woanhyɛ da sɛ wode bɛmena no nkɔ. Sika dodow ne memos a ɛwɔ nnwuma a wɔabɔ ho ban koraa mu no kɔ so yɛ encrypted.
...
| Nkitahodi dodow a woayɛ wɔ bere tenten mu a wobɛkan | Bandwidth nhyehyɛe koro no ara, a wɔahu wɔ bere tenten mu |
...
Susuw ɔmanfo nhomakorabea bi a wokura atesɛm krataa biara a wɔatintim pɛn no ho hwɛ. Node a ɛyɛ ma yɛ ɔkenkanfo a ɔde nneɛma a wɔakora so no nyinaa kɔ fie. Sika kotoku a emu yɛ hare yɛ ɔkenkanfo a ɔsrɛ nhomakorabea sohwɛfo no sɛ ɔmma no da biara da digest mmom — krataa teateaa a ɛkura nea ɛdɔɔso ara kwa a ɛbɛma wɔahu sɛ ebia biribi fa wɔn ho anaa.

Wɔatoto digest no ano: nhomakorabea sohwɛfo no boaboa ano a ontumi nkenkan nneɛma a ɛho hia wo, na wode w’ankasa safe bue wɔ fie. Ɛno ne compact block no, na nea wobue no yɛ trial-decryption wɔ wo device no so.

Nanso nhomakorabea sohwɛfo no da so ara hu ɔkenkanfo ko a ɔkɔɔ mu, bere bɛn, ne sɛnea wɔde bundle a ɛyɛ den no kɔe. Ɛno ne IP address ne bere — a wotumi hu fi pon no so, ɛmfa ho sɛnea wɔatoto envelope no mu yiye. Tor yɛ nea ɛne nkrasɛm a wɔmmɔ ne din a wɔde bɛmena no yɛ pɛ: nhomakorabea sohwɛfo no da so ara de nhomakorabea koro no ara ma, nanso onnim onii fie bio.
...
Mmoa wɔ Rust nhomakorabea ahorow a Zcash sika kotoku pii si so no mu. zcash_client_backend ka Tor module a wɔasi wɔ so ho [Arti](https://tpo.pages.torproject.net/core/arti/), Tor Rust dwumadie, enti sika kotokuo tumi fa sync, transaction broadcast, ne boɔ hwehwɛ kwan fa Tor so a ɔmfa Tor client a ɔyɛ soronko nkɔ.
...
Kɔkɔbɔ abien. Tor de wo IP sie server no, nanso ɛnsesa nea server no sua fi abisade ahorow a woyɛ no mu. Na onion routing de latency ka ho, enti syncing gye bere tenten. W’ankasa server a wode di dwuma no kwati ahotoso asɛmmisa no wɔ ɔkwan soronko so, efisɛ saa bere no, nea ɔyɛ adwuma no ne wo.
...
No [hosh.zec.rocks](https://hosh.zec.rocks/zec) dashboard di ɔmanfoɔ servers ne wɔn akwahosan akyi, na ɛyɛ beaeɛ a wobɛhwɛ deɛ ɛwɔ soro ankasa. [status.zec.rocks](https://status.zec.rocks/) kyerɛ ɔsom tebea.
...
**Adwene sɛ server no betumi akenkan wo nnwuma**. Ɛrentumi nyɛ yiye. Wo safe no tra wo device so, na sika ne memos a ɛwɔ nnwuma a wɔabɔ ho ban koraa mu no tra encrypted — mpo tia ɔtamfo a wasɛe server no.

**Akenkan "shielded" sɛ "anonymous connection"**. Shielded transactions bɔ nea ɛkɔ blockchain no so ho ban. Wo IP address ne bere a wo dwumadi no yɛ layer a ɛyɛ soronko, na saa layer no ne nea server no hu no pɛpɛɛpɛ.

**Sɛ yɛfa no sɛ Tor yi trace biara fi hɔ**. Tor de wo IP sie server no, nanso ɛnsesa nea server no sua firi abisadeɛ a woyɛ mu, na ɛde latency ka syncing ho.

**Seva list a wogye di wɔ wiki krataafa bi so**. Adwumayɛfo ba na wɔkɔ. Hwɛ [hosh.zec.rocks](https://hosh.zec.rocks/zec) ma nea ɛretu mmirika ankasa ansa na wode wo sika kotoku akyerɛ biribiara.

## Tɔfabɔ

Sika kotoku a emu yɛ hare ma wo shielded pool a enni disk space, a ɛyɛ aguadi pa. Ma nea woredi gua no mu nna hɔ kɛkɛ. Server no ntumi nnye wo sika anaasɛ ɛnkenkan wo sika a wɔabɔ ho ban no, nanso ɛwɔ beae pa a wubetumi ahu wo IP address ne bere a woreyɛ adwuma. Fa kwan fa Tor so, hyɛ da paw wo adwumayɛfo, anaasɛ tu mmirika w’ankasa de.
