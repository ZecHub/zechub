# <img src="/content-images/send-svgrepo-com-b62f643de0.svg" width="24" height="24" alt="Journalist icon"/> Send Money Without Linking Your Identity

<span className="inline-flex items-center gap-[6px]"><span className="inline-block w-[12px] h-[12px] bg-green-500 rounded-full"></span>Intermediate - 7 min</span>

## TL;DR

- Bere nyinaa fi **address a wɔabɔ ho ban** so mena.
- Kwati direct t to t nkitahodi ahorow
- Break linkability a ɛda identities ntam
- Ma w’adwene nkɔ bere ne nhwɛso ahorow so

<br/>

## <img src="/content-images/user-svgrepo-com-21adf62b7c.svg" width="24" height="24" className="inline-block align-middle mr-1 p-[2px]" alt="user icon"/> Who is this for?

- Obiara a ɔde sika a ɛho hia mena
- Wɔn a wɔde di dwuma a wɔpɛ kokoam nsɛm
- Nnipa a wɔyɛ adwuma wɔ asiane a wɔhwɛ wɔn so

<br/>

## <img src="/content-images/warning-error-svgrepo-com-b7ea8a50da.svg" width="24" height="24" className="inline-block align-middle mr-1 p-[2px]" alt="warning icon"/> The Problem

Crypto a wɔde mena no taa da no adi sɛ:
- Wo sika kotoku no ho nkyerɛkyerɛmu
- Wo nnwuma ho abakɔsɛm
- Wɔn a wo ne wɔn di nkitaho

Eyi ma wonya **sikasɛm ho mfonini a wotumi hwehwɛ mu**.

<br/>

## <img src="/content-images/icons8-lock-2f8e221321.svg" width="24" height="24" className="inline-block align-middle mr-1 p-[2px]" alt="toolbox icon"/> Why Zcash?


Zcash ma **kokoam transfers koraa** tumi de shielded transactions di dwuma.

Eyi bubu:
- Address linkability a ɛwɔ hɔ
- Ɔmanfo nkitahodi a wɔhwehwɛ

<br/>

## <img src="/content-images/icons8-toolbox-9bebbb1619.svg" width="24" height="24" className="inline-block align-middle mr-1 p-[2px]" alt="toolbox icon"/> What You Need

- Sika kotoku a ɛboa nnwuma a wɔabɔ ho ban
- Ntease titiriw a ɛfa z-address ahorow ho

<br/>

## <img src="/content-images/ladder-svgrepo-com-7232bf46ed.svg" width="24" height="24" className="inline-block align-middle mr-1 p-[2px]" alt="step icon"/> Step-by-Step Guide

### Anamɔn 1: Hwɛ sɛ Wɔabɔ Sika no Ho Ban

Sɛ wo sika wɔ address a ɛda adi pefee so a:
- Fa wɔn kɔ address a wɔabɔ ho ban mu kan


### Anamɔn 2: Fa fi Shielded so kɔ Shielded so
Bere nyinaa pɛ `z to z` asɛm a wɔde di gua

Eyi ma wonya kokoam nsɛm a ɛsen biara.


### Anamɔn 3: Kwati Identity Linkage

NNYƐ:
- Fa nkitahodi ahorow ho amanneɛbɔ ma wɔ baguam
- San fa address koro no ara di dwuma wɔ nsɛm a ɛfa ho nyinaa mu
- Fa address ahorow bata nnipa a wonim wɔn ho

### Anamɔn 4: Susuw Bere Nhyehyɛe Ho

Afotu a ɛkɔ akyiri:
- Kwati sɛ wode bɛmena ntɛm ara bere a woanya sika pii akyi
- Bu nhyehyɛe ahorow a wotumi hyɛ ho nkɔm wɔ baabi a ɛbɛyɛ yiye

<br/>

## <img src="/content-images/icons8-cancel-7f786be3c1.svg" width="24" height="24" className="inline-block align-middle mr-1 p-[2px]" alt="cancel icon"/> Common Mistakes to Avoid

- Sɛ wode address ahorow a ɛda adi pefee so mena tẽẽ 
- Address ahorow a wɔsan de di dwuma wɔ nnipa pii mu 
- Nnwuma a wɔasi so dua wɔ baguam 
- Metadata a wobu ani gu so (bere, mpɛn dodow) .

<br/>

## <img src="/content-images/checked-checkbox-svgrepo-com-7ea19022da.svg" width="28" height="28" className="inline-block align-middle mr-1 p-[2px]" alt="done icon"/> Result

Wobɛtumi:
- Fa sika mena wɔ kokoam
- Kwati sɛ wode wo nipasu bɛbata ho
- Tew sɛnea wotumi hwehwɛ nneɛma mu no so

<br/>

## <img src="/content-images/chain-for-links-svgrepo-com-117ee0dec1.svg" width="24" height="24" className="inline-block align-middle mr-1 p-[2px]" alt="chain-links icon"/> Related
- [Kokoamsɛm - Shielded vs Transparent](/privacy/shielded-vs-transparent)

<br/>

## <img src="/content-images/progress-arrows-svgrepo-com-aad76739e5.svg" width="24" height="24" className="inline-block align-middle mr-1 p-[2px]" alt="progress icon"/> Progress

**Anamɔn 2 wɔ 6 mu**

Seesei wubetumi de sika amena kokoam a worenda wo ho adi.

<br/>

## Anamɔn a Edi Hɔ

- [Freelancer Kokoam Nsɛm Ho Nhyehyɛe](/zcash-use-cases/freelance-privacy-setup)
