<a href="https://github.com/Zechub/zechub/edit/main/site/Zcash_Community/Cypherpunk_Zero_NFT.md" target="_blank">
  <img src="https://img.shields.io/badge/Edit-blue" alt="ページを編集"/>
</a>

![header](/content-images/205220798-c8e51db6-ddad-46ff-b760-401756-c9ba937b4e.webp)


# Cypherpunk Zero

Cypherpunk Zero は、ゼロという若きサイファーパンクハッカーで自由闘士を中心に語られる物語シリーズです。ゼロは現在、独裁的な支配者が社会を縛っているディストピアの悪夢の中に生きていますが、彼女はコードを使ってその支配者たちと戦っています。この物語は Zcash および Halo 暗号技術からインスピレーションを受けました。このクリエイティブなプロジェクトは ECC、Stranger World、Might Jaxx および選定されたエコシステムパートナーによる共同制作です。この作品は特に [NFT シリーズ](https://opensea.io/collection/cypherpunk-zero) に焦点を当てていますが、[プロローグコミック](https://halo.electriccoin.co/#view-prologue) および [コレクタブル玩具](https://mightyjaxx.com/products/cypherpunk-zero) もリリースされています。NFT コミュニティは主に Twitter 上で活動しており、キャンペーンや広範な Zcash コミュニティを支援するプロジェクトの組織化と実行のために [分散型組織（DAO）](https://twitter.com/CypherpunkDAO) を作成しました。

## 抗議ロードマップ

これまで多くの NFT プロジェクトがロードマップで過大な約束をして、実際には成果が伴わないケースを見てきました。そのため、正式なロードマップは存在しません。NFT 持有者向けに素晴らしいユニークな特典が準備されていることを信じてください。その中には予想されるものもあれば、予想外のものもあります。お楽しみにどうぞ。

## ゲッコウ

[Snapshot](https://snapshot.box/#/s:cypherpunkzerodao.eth)

[Guild](https://guild.xyz/cypherpunkzerodao)

[Commonwealth](https://commonwealth.im/cypherpunk-zero)

## 『ストレンジャー・ウルフ』とは？

<div className="my-8 w-full aspect-video max-w-3xl mx-auto rounded-2xl overflow-hidden shadow-lg bg-black">
  <iframe
    className="w-full h-full"
    src="https://www.youtube.com/embed/KF-VwIOYDlE"
    title="Strange Wolf - Awake"
    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
    allowFullScreen
    loading="lazy"
  />
</div>


## NFTはどのチェーンに配置されていますか？

[Ethereum](https://etherscan.io/address/0x3e86d6cf041b719c575f57050697c115f0a53758)

## トークンの配分は？

*総発行量*: Ethereum メインネット上に生成されるユニークな NFT が合計で 10,000 枚
4%（最初の 400 枚）は、Zcash X Mighty Jaxx: Cypherpunk Zero Genesis Edition Toy 59 を購入したプロジェクト初期の支援者にエアドロップされました。

72%（7,200 枚）は、暗号化されたメモをサポートする Zcash ウォレットを使用して行う新しいホワイトリスト機構によって一般公開されます（詳細については後述します）。

残りの 24%（2,400 枚）は ECC が保有し、将来的なイニシアチブに使用される予定です。これは Zcash と Web3 空間におけるプライバシーを推進するためのものです。


# Cypherpunk Zero v2 ホワイトリスト

はい、元々のコレクションはハッキングされました。未来はまだ書き込まれていません。

## 資源

[Cypherpunk Zero ウェブサイト](https://halo.electriccoin.co/)

[Cypherpunk Zero Twitter](https://twitter.com/cypherpunkZero)

[Cypherpunk Zero Opensea コレクション](https://opensea.io/collection/cypherpunk-zero)

[Cypherpunk Zero DAO Twitter](https://twitter.com/CypherpunkDAO)

[Cypherpunk Zero DAO Discord](https://discord.com/invite/sjfgXys4Jf)

[Cypherpunk Zero NFT メガスレッド](https://forum.zcashcommunity.com/t/cypherpunk-zero-nft-megathread/41502?u=dismad)
