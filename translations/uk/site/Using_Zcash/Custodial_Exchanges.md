<a href="https://github.com/zechub/zechub/edit/main/site/Using_Zcash/Custodial_Exchanges.md" target="_blank">
  <img src="https://img.shields.io/badge/Edit-blue" alt="Edit Page"/>
</a>

# <img src="/content-images/image-2024-02-03-173258092-a5440e5ee2.webp" alt="Alt Text" width="50"/>   Кастодіальні біржі

Ironwood — це екранований пул, що з’явився з оновленням мережі в липні 2026 року. Біржі, які працюють лише з прозорими t-адресами, ніколи з ним не взаємодіють, тому для них зазначено «не застосовується». Якщо біржа працює з екранованими адресами, але не повідомила, який пул використовує, це вказано замість припущень.

---

### [Backpack](https://backpack.exchange)

<a href="https://backpack.exchange">
    <img src="/content-images/_unavailable.svg" alt="Backpack Logo" width="200" height="100"/>
</a>

- Вебсайт: [Backpack](https://backpack.exchange)
- Пари: ZEC/USDC | ZEC-PERP
- Підтримка: Депозити та виведення коштів увімкнено. Типи адрес не вказано
- Час депозиту: Не вказано
- Ironwood: Не вказано
___

### [Binance](https://binance.com)

<a href="https://binance.com">
    <img src="/content-images/_unavailable.svg" alt="Binance Logo" width="200" height="100"/>
</a>

- Вебсайт: [Binance](https://binance.com)
- Пари: ALL/ZEC
- Підтримка: Прозорі (депозит на TEX Address)
- Час депозиту: 20 хвилин
- Ironwood: Не застосовується, лише прозорі адреси
___

### [BitcoinVN](https://bitcoinvn.io)

<a href="https://bitcoinvn.io">
    <img src="/content-images/_unavailable.svg" alt="BitcoinVN Logo" width="200" height="100"/>
</a>

- Вебсайт: [BitcoinVN](https://bitcoinvn.io)
- Пари: BTC/ZEC, ETH/ZEC, USDT/ZEC, XMR/ZEC, LTC/ZEC, DASH/ZEC
- Підтримка: Прозорі | Екрановані (депозит і виведення на z-адреси)
- Час депозиту: Миттєво (некостодіальний обмін)
- Ironwood: Не вказано
___

### [Bitfinex](https://bitfinex.com)

<a href="https://bitfinex.com">
    <img src="https://upload.wikimedia.org/wikipedia/en/4/41/Bitfinex_Logo_light.svg" alt="Bitfinex Logo" width="200" height="100"/>
</a>

- Вебсайт: [Bitfinex](https://bitfinex.com)
- Пари: ALL/ZEC
- Підтримка: Прозорі
- Час депозиту: 25 хвилин 
- Ironwood: Не застосовується, лише прозорі адреси
___

### [Coinbase](https://coinbase.com)

<a href="https://coinbase.com">
    <img src="/content-images/coinbase-8643659341.webp" alt="" width="200" height="100"/>
</a>

- Вебсайт: [Coinbase](https://coinbase.com)
- Пари: ALL/ZEC
- Підтримка: Прозорі
- Час депозиту: 150 хвилин
- Ironwood: Не застосовується, лише прозорі адреси
___

 ### [Gemini](https://gemini.com)

<a href="https://gemini.com">
    <img src="/content-images/Gemini-Symbol-ddd31ee98b.webp" alt="Gemini Custodian Exchange Logo" width="200" height="100"/>
</a>

- Вебсайт: [Gemini](https://gemini.com)
- Пари: ALL/ZEC
- Підтримка: Прозорі | Екрановані та уніфіковані виведення коштів, Orchard з листопада 2025 року
- Час депозиту: 50 хвилин 
- Ironwood: Підтримуються екрановані виведення коштів, пул не вказано
___

### [Huobi](https://huobi.com)

<a href="https://huobi.com">
    <img src="/content-images/huobi-global-logo-82DAA48E43-seeklogo.co-bc59e6f286.webp" alt="Huobi Logo" width="200" height="100"/>
</a>

- Вебсайт: [Huobi](https://huobi.com)
- Пари: ALL/ZEC
- Підтримка: Прозорі
- Час депозиту: 35 хвилин 
- Ironwood: Не застосовується, лише прозорі адреси
___

### [Kraken](https://kraken.com)

<a href="https://kraken.com">
    <img src="/content-images/kraken-logo-7407ab477f.webp" alt="Kraken Promotional" width="200" height="100"/>
</a>

- Вебсайт: [Kraken](https://kraken.com)
- Пари: ALL/ZEC
- Підтримка: Прозорі
- Час депозиту: 60 хвилин
- Ironwood: Не застосовується, лише прозорі адреси
___

### [KuCoin](https://kucoin.com)

<a href="https://kucoin.com">
    <img src="/content-images/kucoin-ee0c21f9fd.svg" alt="KuCoin Logo" width="200" height="100"/>
</a>

- Вебсайт: [KuCoin](https://kucoin.com)
- Пари: ALL/ZEC
- Підтримка: Прозорі
- Час депозиту: 20 хвилин 
- Ironwood: Не застосовується, лише прозорі адреси
___

## [OKX (раніше OKEx)](https://www.okx.com)

<a href="https://www.okx.com">
    <img src="https://upload.wikimedia.org/wikipedia/commons/8/89/Official_logo_of_OKEx.png" alt="OKX logo (shown under its former OKEx branding)" width="200" height="100"/>
</a>

- Вебсайт: [OKX](https://www.okx.com)
- Пари: ALL/ZEC
- Підтримка: Прозорі
- Час депозиту: 25 хвилин 
- Ironwood: Не застосовується, лише прозорі адреси
___

### [Robinhood](https://robinhood.com)

<a href="https://robinhood.com">
    <img src="/content-images/IMG-5725-7f964c1fdb.webp" alt="Robinhood Logo" width="200" height="100"/>
</a>

- Вебсайт: [Robinhood](https://robinhood.com)
- Пари: USD/ZEC | EUR/ZEC
- Підтримка: Виведення коштів лише на прозорі t-адреси. Депозити ZEC не підтримуються
- Час депозиту: Депозити не підтримуються
- Ironwood: Не застосовується, лише прозорі адреси
___
