<a href="https://github.com/zechub/zechub/edit/main/site/Start_Here/What_is_ZecHub.md" target="_blank">
  <img src="https://img.shields.io/badge/Edit-blue" alt="Edit Page"/>
</a>


# ZecHub nedir?
---

ZecHub, Zcash için merkeziyetsiz bir eğitim merkezidir. [ZecHub](http://zechub.xyz)'ın amacı, topluluk üyelerinin Zcash ekosistemini destekleyen içerikler oluşturmak, doğrulamak ve tanıtmak için birlikte çalışabileceği bir eğitim platformu sunmaktır. Bunu birkaç mekanizma aracılığıyla yapıyoruz.

1. [Eğitimler](https://www.youtube.com/playlist?list=PL6_epn0lASLFF28ePj7P5TvtMG_WE7p3O)
2. [Wiki belgeleri](https://zechub.wiki/)
3. [Topluluk üyeleriyle röportaj yapılan bir podcast](https://www.youtube.com/playlist?list=PL6_epn0lASLHlNCMtUErX8UfaJK6N9K5O).
4. [Haftalık bir Zcash bülteni.](https://zechub.substack.com/)

Eğitimler, kısa biçimli bloglar ve bülten açık kaynaklıdır ve GitHub üzerinde barındırılır; bu da topluluktaki herkesin katkıda bulunabileceği anlamına gelir. Bu, içeriğin doğru olmasını ve kanalın kendisinin tek bir hata noktasından etkilenmemesini sağlamak içindir.

Zcash'e yeniyseniz, [bu kılavuzu](/start-here/new-user-guide) kullanın.****

## ZecHub üzerinde nasıl değişiklik yapılır

---

1. [ZecHub'ın Github deposuna](https://github.com/ZecHub/zechub) gidin
   
2. Yeni bir Github hesabı oluşturun veya kendi hesabınızla giriş yapın
   
3. Pull request'in nasıl yapılacağını öğrenmek için bu kılavuzu takip edin:

<div className="my-8 w-full aspect-video max-w-3xl mx-auto rounded-2xl overflow-hidden shadow-lg bg-black">
  <iframe
    className="w-full h-full"
    src="https://www.youtube.com/embed/8eYDTyV39a4"
    title="How to Contribute to ZecHub!"
    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
    allowFullScreen
    loading="lazy"
  />
</div>


## Katkıda Bulunanlar İçin Yönergeler

---

ZecHub'ın tutarlı bir görünüme sahip olmasını sağlamak amacıyla, depoya sayfa eklerken kullanabileceğiniz bir şablon vardır.

```markdown
1. All pages should have additional resources (ie. links to z.cash) to support content
2. No marketing or sponsored content
3. No inappropriate content.
```

Bağışlar: [burada](https://explorer.zec.rocks/ua/u17sawf3pdh7z6tsy83vhjd2z7sztpy9y87wyyqz4vqshn3yhch28q3ud3lr0etadl3flz4c9zdrtn9lwlf23vzmjnll97efxzg9h0rjzn9qvhvqr8p3394ysnfhg560xxxc66yxzh8r548myxn3l2qrc9pfrt8nrzhxvlpuuykv7czjsm)

<a href="https://explorer.zec.rocks/ua/u17sawf3pdh7z6tsy83vhjd2z7sztpy9y87wyyqz4vqshn3yhch28q3ud3lr0etadl3flz4c9zdrtn9lwlf23vzmjnll97efxzg9h0rjzn9qvhvqr8p3394ysnfhg560xxxc66yxzh8r548myxn3l2qrc9pfrt8nrzhxvlpuuykv7czjsm">
    <img src="/content-images/image-2023-11-18-141228046-644b4c542d.webp" alt="" width="200" height="200"/>
</a>



**SORUMLULUK REDDİ: ZecHub, Zcash topluluğu üyeleri tarafından kurulmuş tamamen bağımsız ve açık kaynaklı bir girişimdir. Bu Github deposunda yer alan hiçbir şey finansal veya yatırım tavsiyesi olarak değerlendirilmemelidir - yalnızca bilgilendirme amaçlıdır.**

---

*Bu wiki sayfasına ekleme yapmak veya düzenleme önermek isterseniz, lütfen [ZecHub github deposuna](https://github.com/ZecHub/zechub) gidin ve bir pull request gönderin.*
