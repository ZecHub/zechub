...
* It is benchmark-driven: the author publishes indexing and serving benchmarks, including a full mainnet sync from genesis to serving in 92 seconds on the benchmark machine.
...
Most light wallet servers are a separate process that follows a full node. Ztreamer collapses the two: the Zakura node runs embedded in the same Rust process as the indexer and the gRPC server. There is no second daemon to configure, no RPC connection between node and indexer to secure, and no separate chain database — the index is built from the embedded node's data.

On top of the standard light wallet protocol, Ztreamer adds a `CompactTxStreamer` server over v2 p2p. In practice this means light wallets can fetch compact blocks peer-to-peer instead of only from a central server operator.
...
| Node relationship | Separate process; talks to zcashd, Zebra, or Zakura over JSON-RPC | Separate process; reads from Zebra (or zcashd) | Separate services over its own RocksDB projections | Zakura node embedded in-process — no separate node to run |
| Light wallet protocol | Reference implementation | Compatible gRPC endpoint | Compatibility plane (`-compat-lightwalletd`) | All methods implemented, two documented deviations |
...
| Maturity | Oldest; v0.5.4 (Aug 2026) | v0.8.0 (Aug 2026) | Early; 2026 | Newest; v0.1.0 (Sep 2026) |
