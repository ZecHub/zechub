...
> Status: Activated. NU6.2 went live on Zcash mainnet at block 3,364,600 (June 3, 2026 UTC). Some dashboards show it as June 2 in local time, which is the same block and the same moment.
...
The flaw was a soundness bug in Orchard's Action circuit, the part of the math that is supposed to prove every shielded transaction obeys the rules. It had been quietly present since Orchard launched in May 2022. On May 29, 2026, independent security researcher Taylor Hornby found it during a protocol audit for [Shielded Labs](../Zcash_Organizations/Shielded_Labs.md) and disclosed it responsibly.

1. The bug sat unnoticed for about four years.
2. A soundness bug means an invalid transaction could pass the proof check as if it were valid, which in theory could mint counterfeit ZEC.
3. Because that threatens the honesty of the whole supply, the network acted within days.

For a closer look at what the bug was and how it could create value, see the [Ironwood](Ironwood.md) page.
...
| Soft fork | A backward-compatible rule change; here, one that paused Orchard without splitting the network |
...
Why pause instead of patching quietly? A soundness bug threatens the whole supply, so the safe move was to stop new Orchard transactions until the corrected circuit was live.
...
Because the flaw was a soundness bug, meaning invalid transactions could pass as valid. Leaving Orchard open would have left a door to counterfeit ZEC open too. Pausing new Orchard transactions closed that door until the corrected circuit activated at NU6.2.
</details>
