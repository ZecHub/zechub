...
| **CipherPay:** | A payment tool that enables merchants, creators, and communities to accept private Zcash payments. Non-custodial design that Users can generate [payment links or QR codes](https://cipherpay.app) to receive ZEC directly into their wallets. |
| **CypherScan:**| A blockchain explorer that allows users to view transactions, blocks, and network activity on Zcash. |
...
| **Halo 2:** | A "trustless" recursive proof system. It allows the network to stay private without needing the initial "ceremony" to create secret parameters. |
...
| **Intent-Based Swaps:** | A decentralized way to exchange ZEC for other assets. Instead of a traditional order book, users broadcast an "intent", which is then filled by a liquidity provider.|
...
| **Investment:** |   A number of Financial options are available to institutional investors or family offices seeking exposure to Zcash. Full list of Custodians/Investment Vehicles/Trading Firms [here](https://z.cash/investors/)|  
...
|   Term   |  Definition  | 
|----------|--------------|
| **Key Phrase:** |A human-readable sequence of words (sometimes called a mnemonic phrase) used to recover a wallet and its private keys. Key phrases allow users to back up and restore wallets safely without storing raw private keys.|
...
|   Term   |  Definition  | 
|----------|--------------|
| **Mempool (Memory Pool):** | The "waiting room" for transactions. When you send ZEC, it sits in the mempool until a miner includes it in a new block.|
...
|   Term   |  Definition  | 
|----------|--------------|
| **Nighthawk:** |   A Mobile wallet for Zcash - [Website](https://nighthawkwallet.com)| **NSM (Network Security Model)
| **NSM (Network Security Model):** | A formal framework used by Zcash to evaluate and ensure the security of the network, including consensus safety, shielded transaction integrity, and node participation assumptions.|
...
| **NU6:** |The 7th major Network Upgrade for Zcash. NU6 introduces protocol improvements, infrastructure updates, and governance changes that continue the evolution of the Zcash network following NU5.|
...
| **Privacy Set:** | The total number of shielded transactions in a pool (like Orchard). A larger "privacy set" makes it harder for analysts to use timing or patterns to guess who is transacting.|
...
| **Trailing Finality Layer (TFL):**| A mechanism in Zcash that ensures transactions are considered final only after a configurable number of subsequent blocks, providing extra security against chain reorganizations.|
...
| **Trxn ID (Transaction ID):** | A unique identifier for a transaction on the Zcash blockchain. It allows users to track, verify, and reference transactions on block explorers or within wallets.
...
| **UVK (Unified Viewing Key):** | A viewing key for a Zcash Unified Address that allows the holder to view transactions across all receivers (transparent, Sapling, or Orchard) associated with that address. |
...
|   Term   |  Definition  | 
|----------|--------------|
| **Z3 Stack:** | The modern Zcash software stack composed of Zebra (full node), Zaino (indexer), and Zallet (wallet). Designed to replace legacy infrastructure like zcashd and lightwalletd with a more maintainable, performant Rust-based ecosystem.|
| **Zaino:**|A blockchain indexer that works with Zebra to provide fast and efficient blockchain data access for wallets and applications.|
| **Zallet:** | A next-generation Zcash wallet designed to work with the Z3 Stack and provide a modern shielded transaction experience.|
...
| **ZecHub Sync:**| A recurring community call organized by ZecHub where contributors discuss ecosystem updates, development progress, educational initiatives, and community tasks.|
...
| **ZingoCLI:** | A command-line interface wallet for Zcash developed by the team behind Zingo Wallet. It allows developers and advanced users to send shielded transactions, manage addresses, and interact with the Zcash network from the terminal. |
| **Zingo-PC:** | A desktop version of Zingo Wallet designed for personal computers. It provides a graphical interface for sending and receiving shielded Zcash transactions while maintaining privacy by default.|
| **ZKAV Club:** | The Zero-Knowledge Audio/Visual (ZKAV) Club, a group that explores zero-knowledge technology and privacy topics through audio/video discussions, demos, and presentations related to Zero‑Knowledge Proof and Zcash.|
...
| **ZODL:** (formerly Zashi) | A mobile wallet for Zcash focused on simple shielded payments and privacy-by-default UX. Originally released as Zashi and later rebranded to ZODL, the wallet is developed by the Zcash Open Development Lab.|
| **ZSAs (Zcash Shielded Assets)** | ZSAs allow other assets (like stablecoins or NFTs) to be issued natively on the Zcash blockchain with the same privacy protections as ZEC.|
